...
- [ ] `components/ledger.core/src/stevetrading/ledger/core.lpy` (new): `empty-ledger`; two explicit layers:
  - `derive-ledger-facts` — pure: `(derive-ledger-facts ledger-state fill-fact intent-ctx)` → vector of derived ledger facts (`:fact/position-lot-opened`, `:fact/position-lot-closed`, `:fact/cash-movement-recorded`). **Deterministic identity:** every derived fact carries a deterministic `:fact/dedupe-key` — `"lot-open/<fill-id>"`, `"lot-close/<fill-id>/<source-lot-id>"`, `"cash/<fill-id>/<category>"` — and `:record/causation-ids [<fill fact-id>]`, so re-derivation after restart appends nothing new (fact-store dedupe absorbs it).
...
- [ ] `bases/engine-live/src/stevetrading/base/engine_live.lpy` (extend stub): `run-live` — control-plane gate FIRST (load active revision; validate own process id + routes; **refuse to start** on any blocking violation or active kill switch, emitting `:fact/control-plane-violation-recorded`); wire chosen broker (`:broker/alpaca-paper` or `:broker/sim` from manifest route) + market data source + stores + observability; same engine loop as replay base (shared loop fn extracted to `engine.commands` or a small `engine.loop` ns — replay and live MUST share the loop per POLYLITH §10.5); `-main` reads `--control-plane-root`, `--process-id`, `--out-dir`.
  ...
  - **Stale-feed safety gate:** `run-live` MUST refuse to start when the routed broker is `:broker/alpaca-paper` (or any non-sim broker) while the injected market-data source identifies as `:market-data/replay`, unless the operator passes an explicit `--allow-replay-feed` override flag. Refusal emits `:fact/control-plane-violation-recorded` with reason `:violation/replay-feed-with-real-broker`. Rationale: replay fixtures are stale data; pairing them with a real (even paper) broker risks orders from fiction. In-spec engine-live E2E tests therefore run **sim-broker routes only**.
- [ ] Tests:
