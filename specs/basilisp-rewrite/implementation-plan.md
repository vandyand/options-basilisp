...
- **Namespace/file mapping:** ns `stevetrading.domain.identifiers` → `components/domain.identifiers/src/stevetrading/domain/identifiers.lpy`. Hyphenated ns segments map to underscored dirs (`market-data` ns segment ↔ `market_data` dir). Test ns is relative to `tests/` (file `tests/domain/test_schemas.lpy` declares ns `domain.test-schemas`).
  - **Phase 0 finding:** this convention requires `"tests"` listed in pyproject `pythonpath` AND no `tests/__init__.py`. With `__init__.py` present, the basilisp testrunner imports the file as `tests.test_foo` (via the `"."` sys.path root) and rejects it for not matching the declared ns. Phase 0 removed `tests/__init__.py` and prepended `"tests"` to `pythonpath` — do NOT recreate `__init__.py` files under `tests/` (nested dirs work as namespace packages; verified `tests/domain/test_probe_nested.lpy` ns `domain.test-probe-nested` discovers and passes).
...
Goal: engine state shape, the four state machines as data tables, and the pure fact fold. **Status: COMPLETE (commit 8e5b336)** — note: resolved orders stay in :engine/unresolved-orders with :order/resolved? true (lineage lookup in one place); partial-fill+late-cancel-ack stays partially-filled per table-strict reading. Normative: ENGINE_STATE_AND_RECOVERY §3–7, ENGINE_STATE_MACHINES (whole doc).

- [x] `components/engine.state/src/stevetrading/engine/state.lpy` (extend stub): full state shape per ENGINE_STATE_AND_RECOVERY §4 — `:engine/run` (run-id, engine-version, mode, started-at, control-plane revision), `:engine/strategies` (per-strategy: lifecycle state, signal-local state, last-processed market ts, last decision ts, posture), `:engine/unresolved-orders` (keyed by order-intent-id; fields per §4.3), `:engine/portfolio` (derived cache), `:engine/watermarks` (`:watermark/fact-log`, `:watermark/market`, `:watermark/broker`, `:watermark/control-plane`), `:engine/health` (level, active degradations). Constructors: `fresh-state`, `init-run`, accessors/updaters (`strategy-state`, `unresolved-order`, `record-watermark`).
- [x] `components/engine.machines/src/stevetrading/engine/machines.lpy` (extend stub): transition tables AS DATA — one vector of `{:machine :machine/order :from :acknowledged :trigger :fact/fill-observed :guard :guard/remaining-qty-zero :to :filled :effects [...]}` rows transcribing ENGINE_STATE_MACHINES §4.2, §5.2, §6.2, §7.2 **completely** (every row, including cancel races and `orphaned-needs-reconcile` rows). `transition` fn: `(transition machine-id current-state trigger guard-ctx)` → `{:transition/applied? true :transition/to ... :transition/effects [...]}` or `{:transition/applied? false :transition/reason :illegal|:no-row|:guard-failed}`. Guards are keywords resolved in a guard-fn map (e.g. `:guard/remaining-qty-zero` reads `guard-ctx`). Duplicate-fact convergence: a trigger that would re-enter the current terminal state returns `applied? false, reason :duplicate` without effects (§8.1).
- [x] `components/engine.fold/src/stevetrading/engine/fold.lpy` (new component): `fold-fact` — pure `(fold-fact state fact)` → `{:state state' :emitted-facts [...] :pending-commands [...]}`. Dispatch on `:record/type`:
...
Goal: fixture-driven canonical observation stream. **Status: COMPLETE (commit fcade1a)** — v0.2 fixture: closes dip bars 1-3, rise 4-7, fall 8-10 so SMA(3)/SMA(5) genuinely crosses at bar 6 and reverts at bar 10. Normative: MARKET_DATA_ADAPTER_CONTRACTS (whole doc).

- [x] `components/market-data.protocol/src/stevetrading/market_data/protocol.lpy` (extend stub): `defprotocol MarketDataSource` — `(source-id [this])` → provider keyword (e.g. `:market-data/replay`) — REQUIRED identity contract, consumed by the Phase 10 stale-feed gate; `(next-observations [this cursor max-n])` → `{:observations [facts] :cursor next-cursor :exhausted? bool}`; `(source-health [this])` (health map must also include `:source/provider` = `(source-id this)`). Observations are canonical facts with provenance (`:data/provider`, `:data/provider-ref`, `:data/received-at`, quality map) per §4–5.
- [x] `components/replay.fixture/src/stevetrading/replay/fixture.lpy` (new): fixture v0.2 EDN format: `{:fixture/id ... :fixture/version 2 :fixture/inputs [{:input/at "..." :input/type :fact/market-bar-observed :input/payload {...}} ...] :fixture/expected-outputs [{:record/type ...} ...]}`. `load-fixture` (read + validate: inputs sorted by `:input/at`, payloads validate as their fact types), `write-fixture`.
- [x] Create `resources/fixtures/replay/simple-paper-session-v0.2.edn`: ~10 one-minute SPY bars (rising then falling closes so the Phase 7 SMA-crossover strategy enters long then flattens) + 1 calendar session fact + expected-outputs listing the fact types from the v0.1 fixture (prediction-produced, signal-decision-produced, risk-decision-recorded, order-intent-created, broker-ack-accepted, fill-observed). Keep v0.1 file untouched.
- [x] `components/market-data.replay/src/stevetrading/market_data/replay.lpy` (new): `make-replay-source` over a loaded fixture; wraps each input payload in a fact envelope with provider `:market-data/replay`, provider-ref `"<fixture-id>:<index>"`, `:data/received-at` = input timestamp (synthetic timing per §9); cursor = input index.
- [x] Tests `tests/market_data/test_replay.lpy` — provenance completeness on every emitted fact (validate-fact passes; provider/provider-ref/received-at present); cursor pagination; exhaustion; fixture validation rejects unsorted inputs and missing provenance-required fields.
...
Goal: canonical financial folds and derived views. **Status: COMPLETE (commit e651767)** — lot-id = "lot-"+fill-key (account-scoped); recovery replays FULL log for ledger (not snapshot tail; ledger state not snapshotted in v0.1); intent-ctx requires :order-intent/{id,side,account-id,effect}. Normative: LEDGER_AND_PORTFOLIO_MODEL (whole doc).

- [x] `components/ledger.core/src/stevetrading/ledger/core.lpy` (new): `empty-ledger`; two explicit layers:
  - `derive-ledger-facts` — pure: `(derive-ledger-facts ledger-state fill-fact intent-ctx)` → vector of derived ledger facts (`:fact/position-lot-opened`, `:fact/position-lot-closed`, `:fact/cash-movement-recorded`). **Deterministic identity:** every derived fact carries a deterministic `:fact/dedupe-key` that embeds the FULL canonical fill identity (broker context per CANONICAL_SCHEMAS §5.2.5, since `dedupe_key` is globally unique in the fact store) — define `fill-key = <source-system>/<account-id>/<broker-order-id>/<fill-id>` and use `"lot-open/<fill-key>"`, `"lot-close/<fill-key>/<source-lot-id>"`, `"cash/<fill-key>/<category>"` — plus `:record/causation-ids [<fill fact-id>]`, so re-derivation after restart appends nothing new (fact-store dedupe absorbs it) while identical fill-ids from DIFFERENT accounts/brokers never collide. Test: two fills with the same `:fill/id` but different account-ids both produce ledger facts (no silent drop).
  - `fold-ledger-fact` — pure fold consuming the derived ledger facts themselves (NOT fills): lots (lot-id `"lot-" + fill-id`, originating order-intent-id, instrument, open ts, open qty, remaining qty, cost basis from fill price × qty ± fees, strategy-id, account-id per §5); opening vs closing effect from the originating intent's `:order-intent/effect` (via intent-ctx at derivation time); closes consume lots FIFO with partial-close splitting; cash movements: trade principal (sign by side) + fees per §6.
  - **Recovery consistency rule:** because derived ledger facts are durable and folded directly, recovery rebuilds ledger/portfolio state by folding the ledger facts already in the log — `derive-ledger-facts` runs only for fills whose derived facts are not yet appended (dedupe keys make this idempotent). Wire this into `engine.recovery/run-recovery!` in this phase: after fact replay, derive+append any missing ledger facts for recovered fills, fold all ledger facts, refresh `:engine/portfolio`.
- [x] `components/portfolio.core/src/stevetrading/portfolio/core.lpy` (new): `derive-portfolio` from ledger state (+ optional marks map): net positions per instrument, open lots, cash, realized P&L (sum of closed-lot proceeds − basis), unrealized (marks − basis on open lots; nil-safe when no mark), gross/net exposure, per-strategy and per-account views per §7/§11. Output stamps `:portfolio/source-watermark`.
- [x] Tests `tests/ledger/`:
  - `test_ledger.lpy` — open fill creates lot + principal cash movement; partial close consumes FIFO with exact lineage (closed portion references original lot + closing intent); fees reduce cash; Decimal-string in/out everywhere (no floats); **idempotent re-derivation:** running `derive-ledger-facts` twice for the same fill and appending through the fact store yields no duplicate ledger facts and `=` ledger state.
  - `test_portfolio.lpy` — realized P&L on a round trip = (sell − buy) × qty − fees; unrealized from marks; rebuild-from-facts: fold full fact list twice → `=` portfolio; recovery path (`run-recovery!`) over a log containing fills + derived ledger facts rebuilds a non-stale `:engine/portfolio`.
...
Goal: the imperative shell + first full session. **Status: COMPLETE (commit 23c5494)** — one observation/cycle; only store-accepted facts are folded (duplicate-bar protection); broker clock = market time atom (restart convergence); resume auto-detects facts.db+snapshot; :max-facts = crash hook; :engine/cursors added to state (loop-side, fold-untouched). Normative: ARCHITECTURE §9 (canonical runtime model), EVENT_COMMAND_TAXONOMY §10, ENGINE_STATE_AND_RECOVERY §6.

- [x] `components/engine.commands/src/stevetrading/engine/commands.lpy` (new): `derive-commands` (pure): from intents/decisions → `:command/submit-order-intent` envelopes (idempotency-key `"submit/intent/" + order-intent-id`, target-adapter `:adapter/broker`); `dispatch-commands!` (impure): routes commands to adapters (broker protocol), emits `:fact/command-dispatched` before adapter call and appends adapter-returned facts after; submit-dedupe rule: skip dispatch when state proves the intent already accepted/completed (ENGINE_STATE_AND_RECOVERY §12.1).
- [x] `bases/engine-replay/src/stevetrading/base/engine_replay.lpy` (extend stub): `run-replay` — `(run-replay {:fixture-path ... :strategy-paths [...] :artifact-root ... :out-dir ...})`:
  ...
  3. loop: pull observations → append → fold → run pipeline per active strategy (warmup via strategy lifecycle machine: `warming-up` until required-bars consumed, then `active`) → append decision facts → derive/dispatch commands → append outcome facts → fold outcomes → **ledger/portfolio integration point:** every folded `:fact/fill-observed` goes through `ledger.core/derive-ledger-facts` (with the intent ctx from unresolved-order state) → derived ledger facts are appended to the fact store (dedupe keys make this idempotent) → `ledger.core/fold-ledger-fact` folds the appended derived facts into ledger state → `portfolio.core/derive-portfolio` refreshes the engine state's `:engine/portfolio` cache at the end of each cycle, so the NEXT pipeline invocation's risk stage reads a current portfolio-view. (Raw fills are never folded directly into ledger state — only derived ledger facts are, matching the Phase 6 derive/fold split and the recovery path.) → snapshot every N facts (configurable, default 50)
...
- [x] Golden tests `tests/e2e/test_replay_golden.lpy`:
...
Goal: operational boundary + paper-capable assembly. **Status: COMPLETE (commit e46d337)** — shared loop extracted to components/engine.loop (both bases use it; replay golden tests stayed green); sim-route manifest at resources/control-plane/examples/sim-main-v0.1.edn; alpaca adapter is stub-transport only (NO network calls). Normative: CONTROL_PLANE_MANIFESTS (whole doc), OBSERVABILITY (whole doc), BROKER_ADAPTER (whole doc), ADR-0005.

- [x] `components/control-plane.protocol/src/stevetrading/control_plane/protocol.lpy` (new): manifest schema fns — `validate-manifest` (structural §9.1: required fields, malformed/duplicate ids, unknown enums, dangling refs; safety §9.2: live strategies on non-live accounts, paper→live-only accounts, strategies with no route, process mode mismatches; policy §9.3: artifact compat warnings) → `{:valid? bool :errors [...] :warnings [...]}`; `kill-switch-active?` (scope matching: global/process/strategy/account).
- [x] `components/control-plane.file-store/src/stevetrading/control_plane/file_store.lpy` (new): revisions under `<root>/revisions/<rev-id>.edn` (immutable — write refuses overwrite), activation records `<root>/activations/<rev-id>.edn`, `active.edn` pointer per deployment scope; implements `persistence.protocol/ControlPlaneStore` + `activate!` (validate → write activation → flip pointer atomically).
- [x] `bases/control-plane/src/stevetrading/base/control_plane.lpy` (extend stub): `-main` with subcommands `validate <manifest.edn>`, `activate <manifest.edn> --root <dir>`, `show --root <dir>`; exit 1 on invalid; prints structured results.
- [x] `components/observability.protocol/src/stevetrading/observability/protocol.lpy` (new): `defprotocol AuditSink` — `(emit-audit! [this event])`, `(emit-metric! [this metric])`, `(health-view [this])`. Audit event minimum fields per OBSERVABILITY §4.1.
- [x] `components/observability.structured-log/src/stevetrading/observability/structured_log.lpy` (new): JSON-lines sink (python `json` interop; keywords → `"ns/name"` strings) to a file or stdout; in-memory ring buffer for `health-view`; engine integration: every appended fact of an audit-relevant type also emits an audit event (decision, order, recovery, lifecycle, degradation, fault).
- [x] `components/broker.alpaca/src/stevetrading/broker/alpaca.lpy` (new): implements `broker.protocol/Broker` for Alpaca paper REST:
...
- [x] `bases/engine-live/src/stevetrading/base/engine_live.lpy` (extend stub): `run-live` — control-plane gate FIRST (load active revision; validate own process id + routes; **refuse to start** on any blocking violation or active kill switch, emitting `:fact/control-plane-violation-recorded`); wire chosen broker (`:broker/alpaca-paper` or `:broker/sim` from manifest route) + market data source + stores + observability; same engine loop as replay base (shared loop fn extracted to `engine.commands` or a small `engine.loop` ns — replay and live MUST share the loop per POLYLITH §10.5); `-main` reads `--control-plane-root`, `--process-id`, `--out-dir`.
  ...
  - **Stale-feed safety gate:** `run-live` MUST refuse to start when the routed broker is `:broker/alpaca-paper` (or any non-sim broker) while `(market-data.protocol/source-id source)` returns `:market-data/replay`, unless the operator passes an explicit `--allow-replay-feed` override flag. Refusal emits `:fact/control-plane-violation-recorded` with reason `:violation/replay-feed-with-real-broker`. Rationale: replay fixtures are stale data; pairing them with a real (even paper) broker risks orders from fiction. In-spec engine-live E2E tests therefore run **sim-broker routes only**. Test (in `test_live_gate.lpy`): alpaca-paper route + replay source without the flag → startup refusal with the violation fact; with `--allow-replay-feed` → gate passes (use a stub-transport alpaca broker so no real call occurs).
- [x] Tests:
  ...
  - `tests/broker/test_alpaca.lpy` — stub-transport contract tests: submit single-leg → correct request shape + ack normalization; multi-leg OCC symbols; rejection (403 insufficient buying power JSON) → `:broker-rejection/insufficient-buying-power`; transport exception → fault fact; duplicate submit reuses client_order_id; open-orders/fills-since normalization; **client_order_id lineage cases:** broker-returned order with a `client_order_id` matching an unresolved intent's short id maps back to the full `order-intent-id` in the normalized fact; broker order with an UNKNOWN client_order_id → anomaly fact (no guessed attachment, ENGINE_STATE_AND_RECOVERY case G); two unresolved intents sharing a short id (construct artificially) → reconciliation anomaly for both, never silent pick.
...
- [ ] `tests/recovery/test_crash_scenarios.lpy` — the 7 acceptance scenarios as deterministic tests using the replay base with injected crash points (crash = stop loop, drop in-memory state, `run-recovery!` from disk, continue):
  ...
  4. crash after fill, before snapshot → fact replay restores; no duplicate fill on re-poll; **recovered `:engine/portfolio` equals the uninterrupted run's portfolio** (ledger facts re-folded, not stale)
