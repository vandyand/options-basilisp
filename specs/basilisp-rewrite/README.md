...
- **Deliberate non-product scope:** `.claude/skills/feature/` (polish.json + adversary/retro prompt copies) is feature-lifecycle tooling for this repo's spec workflow — committed intentionally so future `/feature polish`/`/feature retro` runs use repo-appropriate lint/test gates (`scripts/lint.sh`, `scripts/test.sh`) instead of the global babashka defaults. Not part of the trading system.
